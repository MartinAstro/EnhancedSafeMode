...
from Environments.ESM_POMDP import ESM_POMDP
...
def build_agent(env, config, train_step):
    ...
    with strategy.scope():
        ...
        actor_net = actor_distribution_rnn_network.ActorDistributionRnnNetwork(
                    ...
                    output_tensor_spec=action_spec, # action_spec needs .shape.num_elements()
                    input_fc_layer_params=config['actor_fc_layer_params'],
                    output_fc_layer_params=config['actor_fc_layer_params'],
                    activation_fn=config['activation_fcn'],
                    lstm_size=config['lstm_size']
            )
...
def main():
    policy_name = "PINN_Policy_POMDP"
    policy_name = "Poly_Policy_POMDP"
    policy_name = "Simple_Policy_POMDP"
    max_time = 3*60*60
    random_seed = 1
    ...
    for policy_name in policy_name_list:
        ...
        init_collect_env = BatchedPyEnvironment(envs=[wrappers.TimeLimit(Env(planet, pinn_model, reset_type='orbiting', random_seed=random_seed), 10*60)])

        # Generate new training samples using PINN
        collect_env = BatchedPyEnvironment(envs=[wrappers.TimeLimit(Env(planet, grav_model, reset_type='orbiting', random_seed=random_seed), 10*60)])

        # Evaluate performance using the polyhedral model
        eval_env = BatchedPyEnvironment(envs=[wrappers.TimeLimit(Env(planet, pinn_model, reset_type='standard', random_seed=random_seed), 10*60)])
        ...
        # Training the Agent: Includes collecting data from Environments.ESM_MDP and optimizing agent networks
